...
class mf_custom_fields extends mf_admin {
  ...
  function fields_list() {
    ...
    if($post_type->name == "post"){
      $categories = $this->mf_get_categories();
      $cat_id = isset($_GET['cat_id']) ? $_GET['cat_id'] : '';

      // categories select
      print '<span class="mf-change-cat" >'.__( 'Choose a Category', $mf_domain ).' </span>';
      print '<select id="change-cat" >';
      printf('<option value="%s" >%s</option>','','all');
      foreach($categories as $cat){
        $selected = '';
        if($cat->term_id == $cat_id ) $selected = 'selected="selected"';
    ...
    print '<a href="admin.php?page=mf_dispatcher&mf_section=mf_custom_group&mf_action=add_group&post_type='.$post_type->name.'&cat_id='.$cat_id.'" class="add-new-h2 button">+ '.__( 'Create a Group', $mf_domain ).'</a>';
    print '<a href="admin.php?page=mf_dispatcher&mf_section=mf_custom_fields&mf_action=add_field&post_type='.$post_type->name.'&cat_id='.$cat_id.'" class="add-new-h2 button">+ '.__( 'Create a Field', $mf_domain ).'</a>';
    print '</p>';
    //list cusmtom field of post type
    $groups = $this->get_groups_by_post_type($post_type->name, $cat_id);      
    
    if( empty( $groups ) ) :
    ?>
      <div class="message-box info">
        <p>
          This post type haven't any custom field yet,  create one <a href="admin.php?page=mf_dispatcher&mf_section=mf_custom_fields&mf_action=add_field&post_type=<?php print $post_type->name;?>&cat_id=<?php echo $cat_id;?>">here</a> or
          you can create a group <a href="admin.php?page=mf_dispatcher&mf_section=mf_custom_group&mf_action=add_group&post_type=<?php print $post_type->name;?>&cat_id=<?php echo $cat_id;?>">here</a>
        </p>
      </div>
    <?php
    endif;
    ...
    if($name != 'Magic Fields'){
      $name = sprintf('<a class="edit-group-h2" href="admin.php?page=mf_dispatcher&mf_section=mf_custom_group&mf_action=edit_group&custom_group_id=%s&cat_id=%d">%s</a>',$group['id'],$cat_id,$name);
    
      $add = sprintf('admin.php?page=mf_dispatcher&mf_section=mf_custom_fields&mf_action=add_field&post_type=%s&cat_id=%d&custom_group_id=%s',$post_type->name,$cat_id,$group['id']);

      $name .= sprintf('  <span class="mf_add_group_field">(<a href="%s">create field</a>)</span>',$add);

      $delete_link = 'admin.php?page=mf_dispatcher&init=true&mf_section=mf_custom_group&mf_action=delete_custom_group&custom_group_id='.$group['id'].'&cat_id='.$cat_id;
      ...
      $name .= sprintf( '  <span class="mf_add_group_field">(<a href="admin.php?page=mf_dispatcher&mf_section=mf_custom_fields&mf_action=add_field&post_type=%s&cat_id=%d">create field</a>)</span>',$post_type->name,$cat_id );	
    }
    ...
    ?>
        ...
        <tr id="order_<?php echo $field['id']; ?>">
          ...
          <td>
            ...
            <div class="row-actions">
              <span class="edit">
               <a href="admin.php?page=mf_dispatcher&mf_section=mf_custom_fields&mf_action=edit_field&custom_field_id=<?php echo $field['id'];?>&cat_id=<?php echo $cat_id; ?>"><?php _e('Edit',$mf_domain); ?></a>
              </span> |
              <span class="delete">
                <?php
                $delete_link = "admin.php?page=mf_dispatcher&mf_section=mf_custom_fields&mf_action=delete_custom_field&cat_id=$cat_id; &custom_field_id={$field['id']}&init=true";
     ...
     <?php else:?>
      <div class="message-box info">
        <p>
          This group haven't any custom field yet,  create one <a href="admin.php?page=mf_dispatcher&mf_section=mf_custom_fields&mf_action=add_field&post_type=<?php print $post_type->name;?>&cat_id=<?php echo $cat_id;?><?php if($group['label'] != "Magic Fields"){?>&custom_group_id=<?php print $group['id'];}?>">here</a>
        </p>
      </div>
     <?php endif; ?>
      <br />
  ...
  function save_custom_field(){
    ...
    $this->mf_redirect('mf_custom_fields','fields_list',array('message' => 'success','post_type' => $mf['core']['post_type'],'cat_id'=>$mf['core']['cat_id']));
  }
  ...
  public function fields_form() {
    ...
    $data = array(
      'core'  => array(
        ...
        'cat_id' => array(
          'type' => 'hidden',
          'id'   => 'customfield-cat_id',
          'name' => 'mf_field[core][cat_id]',
          'value' => $cat_id
        ),        
    ...
    <h2><?php _e('Edit Custom Field', $mf_domain); echo ' - '.$data['core']['label']['value'];?></h2>
     ...
     <form id="addCustomField" method="post" action="admin.php?page=mf_dispatcher&init=true&mf_section=mf_custom_fields&mf_action=save_custom_field" class="validate mf_form_admin">
      <div class="alignleft fixed" style="width: 40%;" id="mf_add_custom_field">
        ...
        <?php endforeach;?>
        <p class="submit">
          <a style="color:black" href="admin.php?page=mf_dispatcher&mf_section=mf_custom_fields&mf_action=fields_list&post_type=<?php echo $data['core']['post_type']['value'];?>&cat_id=<?php echo $data['core']['cat_id']['value'];?>" class="button">Cancel</a>
