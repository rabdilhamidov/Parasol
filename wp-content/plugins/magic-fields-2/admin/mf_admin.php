...
class mf_admin {
  ...
  public function mf_get_categories($_args = array()){
    global $wpdb;
   ...
   */
  public function get_groups_by_post_type($post_type, $cat_id=0){
    global $wpdb;

    $query = sprintf("SELECT * FROM %s WHERE post_type = '%s' AND category_id = '%d' ORDER BY id",MF_TABLE_CUSTOM_GROUPS,$post_type,$cat_id);
   ...
   */
  public function get_default_custom_group($post_type, $cat_id=0){
    global $wpdb;

    $query = sprintf("SELECT id FROM %s WHERE name = '__default' AND post_type = '%s' AND category_id = %s ",MF_TABLE_CUSTOM_GROUPS,$post_type,$cat_id);
  ...
  public function new_custom_group($data){
    ...
    $sql = $wpdb->prepare(
      ...
      " (name, label, post_type, category_id, duplicated, expanded) ".
      " VALUES (%s, %s, %s, %d, %d, %d)",
      $data['core']['name'],
      $data['core']['label'],
      $data['core']['post_type'],
  ...
  public function new_custom_field($data){
    ...
    if(!$data['core']['custom_group_id']){
      $custom_group_id = $this->get_default_custom_group($data['core']['post_type'], $data['core']['cat_id']);
    ...
    $sql = $wpdb->prepare(
      ...
      " (name, label, description, post_type, category_id, custom_group_id, type, required_field, duplicated, options) ".
      " VALUES (%s, %s, %s, %s, %d, %d, %s, %d, %d, %s)",
      $data['core']['name'],
      $data['core']['label'],
      $data['core']['description'],
      $data['core']['post_type'],
